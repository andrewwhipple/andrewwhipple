...
var podcasts_json = require('./podcasts.json');
...
function podsClear(err) {
  if(err) console.log(err);

  // loop over the projects, construct and save an object from each one
  // Note that we don't care what order these saves are happening in...
  var to_save_count = podcasts_json.length;
  for(var i=0; i<podcasts_json.length; i++) {
    var json = podcasts_json[i];
    var pod = new models.Podcast(json);

    pod.save(function(err, proj) {
      if(err) console.log(err);
